...
def extract_face(img, box, image_size, margin, save_path, remove_eyes=False):
    if remove_eyes:
        ...
        for rect in rects:
            ...
            dist = x2 - x1
            dist_multiplier = 0.15
            new_x1 = int(x1 - dist_multiplier * dist)
            new_x2 = int(x2 + dist_multiplier * dist)

            new_y1 = y1 + int(dist_multiplier * dist)
            new_y2 = y2 - int(dist_multiplier * dist)

            cv2.rectangle(img, (new_x1, new_y1), (new_x2, new_y2),
                          (0, 0, 0), -1)
    ...
    if save_path is not None:
        ...
        print("EXTRACT FACE THING:", save_path)
